...
void prepare(void) {
  ...
  // ex 12.1: since stdout is terminal, it is line-buffered and thus
  // no duplicate output occurs. When stdout is a file, it is fully buffered and we see a duplicate output.
  // If we removed the newline in the following string, we can see duplicate output even if stdout is terminal.
